...
ステータス: **Phase 1〜3 実装済み（作成・出題・正誤記録まで動作）／ Phase 4〜5 未着手**

> 実装済みぶんの要点は `CLAUDE.md` の「自作問題集（my問題集）」の節にまとめてある。
> このファイルは**未着手ぶん（Phase 4・5）の設計と、そこに至る判断の記録**として読むこと。
...
| 4 | 盤面インポート層（`importBoard.js`） | **未着手** | 2 | 小 | 純粋関数・テスト容易 |
...
| 6 | Phase 4: インポート層 | 牌姿テキストから盤面が再現される。`importBoard.test.js` が通る | 未着手 |
| 7 | Phase 5: 牌譜パーサ | 牌譜JSONから BoardSnapshot に変換できる。テストで固定 | 未着手 |
| 8 | Phase 5: 局面選択UI | 牌譜を読み込み、局・巡目を選んで盤面に反映 → 正解を設定して保存できる | 未着手 |
