...
4. ✅ **アクセス拒否画面を廃止**（`App.jsx` の `isAllowed === false` の分岐）。
   許可の無いユーザーもカテゴリ画面へ通し、`CategoryList` の `officialLocked` で
   **中身が空の書籍タブを落として**「my問題集」とドリルだけを見せる。案内は `.limited-notice`
   （許可ユーザーの従来挙動＝空タブ＋「非公開のコンテンツです」は変えていない）
5. ✅ 作成画面に残数表示（`src/myproblems/limits.js`）。
   カテゴリ/問題の残数・上限での無効化・解説と注釈の `maxLength` と残数・保存エラーの文言変換
6. ✅ ランディングの `.landing-secondary` を「ログインしてできること」に書き替え、
   my問題集（誰でも・20問まで）と何切る問題集（限定公開）を並記。
   **CTAは増やしていない**（主CTA＝ドリルの1つだけという方針は維持）

**⚠️ 上限値を変えるときは3箇所**: `docs/user-problems-schema.sql`（RLS と CHECK・**防御はここだけ**）、
`src/myproblems/limits.js`（画面の案内）、この計画書。片方だけ変えると
「画面では追加できるのに保存だけ失敗する」状態になる。
