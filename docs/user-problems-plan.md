...
### 一般公開の方針（2026-07-28 決定・実装は公開判断とセット）

my問題集は**一般公開を予定している**。容量の圧迫を防ぐため、公開時に以下を入れる。

| 項目 | 決定 |
|---|---|
| 利用できる人 | **ログイン済みなら誰でも**。`allowed_users` に載っていない人は**ドリルと my問題集だけ**触れる（公式問題は従来どおり限定公開） |
| 1ユーザーの問題数 | **20問** |
| 1ユーザーのカテゴリ数 | **5件** |
| 解説の文字数 | **200字** |
| 問題画像 | **引き続き無効**（`hideImage`）。Storage は行より桁違いに重く、バケットのポリシー整理も要るため |

**上限は必ず DB 側で掛ける。** anon キーは公開されているので REST を直接叩けば insert できてしまい、
アプリのUIで止めても意味がない。`user_problems` / `user_categories` の RLS `with check` に件数条件を、
`explanation` に文字数の CHECK 制約を置く（`feedback` テーブルの CHECK 制約と同じ手口）。
UI 側は「残り N / 20問」の表示と保存ボタンの無効化を**二重で**持たせる（403 のままでは理由が伝わらないため）。

**容量の見積もり**（判断の根拠）: 1問あたり 1.5〜3KB（他家の捨て牌が最も大きい）。
1ユーザー20問＝約 40KB なので、1,000ユーザーでも 40MB 程度に収まる。
**上限値はDB側の定数を1行変えれば広げられる**ので、窮屈なら運用しながら緩める。

**公開に切り替えるときの作業**:

1. ✅ SQL … 件数条件と CHECK 制約（`user-problems-schema.sql` の「一般公開の準備」・2026-07-28 作成）
2. ✅ `MyProblemsApp` の `isAdmin` ゲートを外す（ログイン済みなら通す）
3. ✅ `App.jsx` の `canUseMyProblems` をログイン済み判定に変更
4. ⬜ **`App.jsx` の `isAllowed === false` の分岐**（現在はアクセス拒否画面で行き止まり）を作り替え、
   カテゴリ画面へ通して「my問題集」と「メンチン何切るドリル」のタブだけ見せる ← **公開時の最大の変更点**
   - `groupByBook` は中身が空でも書籍タブを作るので、**未許可ユーザーには公式書籍の空タブが並んでしまう**。
     `CategoryList` に「空の書籍タブを落とす」オプションを足して、未許可ユーザーのときだけ有効にする
     （許可ユーザーの従来挙動＝空タブ＋「非公開のコンテンツです」は変えない）
5. ⬜ 作成画面に残数表示（「残り N / 20問」・上限で追加ボタンを無効化・保存エラーの文言変換）
6. ⬜ ランディング（未ログイン）に my問題集の紹介を載せる（**載せる**と決定・2026-07-28）。
   公開後は「ログインすれば誰でも使える」ので行き止まりにならない。機能カード1枚＋CTAの格上げを検討する

**⚠️ 1〜3 だけ入った状態（2026-07-28 現在）**: DBの上限は効いているが、
未許可ユーザーは本体アプリのアクセス拒否画面で止まるため、`/myproblems.html` を**直接開いた人だけ**が使える。
4 を入れて初めて「ログインすれば誰でも見つけられる」状態になる。
